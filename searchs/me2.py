...
def get_name(query):
    ...
    url=f'https://me.go.kr/home/web/staff/list.do?menuId=10433&searchKey=userNm&searchValue={query}'
    browser.get(url)
    ...
    get_names=browser.find_element(By.ID,'trRow_0')
...
def mes(update:Update,context:CallbackContext):
    user_text=update.message.text
    user_text1=user_text.split(' ')[1]
    names=get_name(user_text1)
    update.message.reply_text(f'환경부에 현재 {user_text1}이 {a}명 있습니다')
    for i in range(0,a):
