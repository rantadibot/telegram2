...
def get_name(query):
    ...
    url=f'https://www.msit.go.kr/empSearch/list.do?searchFlag=Y&pageIndex=1&searchOpt=nm&searchTxt={query}'
    browser.get(url)
...
def msits(update:Update,context:CallbackContext):
    user_text=update.message.text
    user_text1=user_text.split(' ')[1]
    names=get_name(user_text1)
    bot.send_message(chat_id=chat_id, text=f'과기부에 현재 {user_text1}이 {a}명 있습니다')
    for i in range(0,a):
